...
- _Second example:_
	1. Evaluate the _first assignment:_ `X=3+5`
		- In prolog, `3+5` is treated _as a term_, which is not evaluated imediately, unless we use the `is` operator, thus `X=3+5` _remains unevaluated_
	2. Evaluate the _second assignment:_ `Y is 3*X`
		- The `is` operator requires, that `X` _be instantiated to a number_. Since `X = 3+5` is _not a number_, but _a term_, this will __result in an error__.
